    ...
    var FRHeaderLoggedIn = (function () {
        ...
        function bindEvents() {
            ...
            $(document).on('click', function (e) {
                ...
                if (!$headerContainer.find('.notifications-popover #notifications-list').length) {
                    return;
                }

                var $target = $(e.target) || $(e.srcElement);
                var $popover = $target.closest('.popover-notification');
                var $popoverTrigger = $target.closest('.popover-notification-trigger');

                if ($popoverTrigger.length) {
                    e.preventDefault();

                    var popoverRight = $headerContainer.find('.username-container').outerWidth() + $headerContainer.find('.profile-container').outerWidth();
                    var popoverArrowRight = popoverRight + ($headerContainer.find('.notification-container').outerWidth() / 2);

                    $headerContainer.find('.notifications-popover').css('right', popoverRight - 7 + 'px');
                    $headerContainer.find('.popover-notification em').css('right', popoverArrowRight - 7 + 'px');
                    $headerContainer.find('.popover-notification').fadeToggle('fast', 'swing');
                    updateNotifications();
                } else if (!$popover.length) {
                    $headerContainer.find('.popover-notification').hide();
                    $headerContainer.find('.popover-notification .notification-item').removeClass('new-notification-item');
                }
            });
        ...
        function updateNotifications() {
            ...
            $headerContainer.find('.notifications-count').addClass('hidden').hide();

            var count = parseInt($headerContainer.find('.notifications-count').text());
            var lastNotifiedId = parseInt($headerContainer.find('.popover-notification #hdnLastNotificationId').attr('value'));

            if (count <= 0 || isNaN(lastNotifiedId) || lastNotifiedId <= 0) {
                return;
            }

            FRHeaderResources.refreshCacheTimeStamp();
            ...
            $.ajaxCrossDomainAPI({
                ...
                url: window.FRHeaderTaxonomyData.Navigation.FrontiersJournalAPIUrl + '/Header/UpdateNotifications?lastNotifiedId=' + lastNotifiedId,
                success: function () {
                    $headerContainer.find('.popover-notification #hdnLastNotificationId').attr('value', 0);
                }
            });
        }
        ...
        function fillNavigationUrls() {
            ...
            $headerContainer.find('a.username')
                ...
                .attr('href', window.FRHeaderProfileData.Profile.Navigation.UserProfileUrl);
        ...
        function renderNotifications() {
            ...
            var result = window.FRHeaderProfileData.Profile.NotificationInfo;
            ...
            var notificationTemplate = [
                ...
                '<img  alt="img" title="{PROFILEFULLNAME}" data-placement="left" src="{PROFILEIMAGESRC}">',
                '</div>',
            ...
            var notificationFinalHtml;
            var notificationContent = '';

            if (result.Count > 0) {
                $headerContainer.find('.notifications-count').text(result.CountAggregated).removeClass('hidden').show();
            } else {
                $headerContainer.find('.notifications-count').addClass('hidden').hide();
            }

            $headerContainer.find('#hdnLastNotificationId').attr('value', result.Id);

            var notificationDetails = result.Info;

            if (notificationDetails == null || !notificationDetails.length) {
                return;
            }

            for (var i = 0; i < notificationDetails.length; i++) {
                notificationContent += notificationTemplate
                    .replace(/{NOTIFICATIONURL}/g, notificationDetails[i].Url)
                    .replace(/{PROFILEIMAGESRC}/g, notificationDetails[i].ThumbnailUrl)
                    .replace(/{PROFILEFULLNAME}/g, notificationDetails[i].Name)
                    .replace(/{NOTIFICATIONTEXT}/g, notificationDetails[i].Text)
                    .replace(/{DEFAULTIMGSRC}/g, "this.src='" + window.FRHeaderTaxonomyData.Navigation.FrontiersJournalAPIUrl + "/Areas/Header/Content/Images/default-profile-24.jpg'");
            }

            notificationContent = '<div id="notifications-list">' + notificationContent + '</div>';
            var notificationHeader = ['<h2>Notifications</h2>'].join('');

            notificationFinalHtml = notificationHeader + notificationContent;

            var $notificationFinalHtml = $(notificationFinalHtml);
            $notificationFinalHtml.find('.notification-item:lt(' + result.Count + ')').addClass('new-notification-item');
            $headerContainer.find('.notifications-popover').append($notificationFinalHtml);
        }
        ...
        function renderProfileMenus() {
            ...
            if (window.FRHeaderProfileData.Profile.NotificationInfo &&
                window.FRHeaderProfileData.Profile.NotificationInfo.Count > 0) {
                unreadNotificationsCount = '<b>' + window.FRHeaderProfileData.Profile.NotificationInfo.CountAggregated + '</b>';
            }

            var profileImpactMenus = { 'n': 'Impact', 'url': window.FRHeaderProfileData.Profile.Navigation.LoopImpactUrl };
            var profileNotificationMenu = { 'n': 'Notifications ' + unreadNotificationsCount, 'url': window.FRHeaderTaxonomyData.Navigation.SharepointWebsiteUrl + '/MyHome/MyNotification.aspx' };
            var mobileViewProfileMenus = new Array().concat(profileMyHomeMenu, profileNotificationMenu, profileImpactMenus, lstProfileMenus);;
